...
def generate_trans_probs(comorbidity):
    if comorbidity <= 0:
        ...
        # P_after5d[:3] = transition_probs_after5[:3]
    elif comorbidity <= 3:
    ...
    return random_P_before5d


def count_death_rate(P_ij, initial_state, duration):
    values = np.array(initial_state, dtype=float)
    for _ in range(duration):
        values = np.dot(P_ij.T, values)
    return values[4]
...
def bootstrap(comorbidity, initial_state, duration, n_samples=1000, alpha=0.9):
    ...
    print(low_res, samples_res[low_sample_idx])
    print(high_res, samples_res[high_sample_idx])
